...
class DB {
    ...
    findDepartmentBudget() {
        return this.query('SELECT departments.department_id, departments.department_name, SUM(roles.role_salary) as Budget FROM employees left join roles on employees.role_id = roles.role_id left join departments on roles.department_id = departments.department_id group by departments.department_id, departments.department_name')
    }
    ...
    deleteEmployee(employeeId) {
        return this.query(`DELETE FROM employees WHERE employee_id = $1`, [employeeId])
    }
    deleteRole(roleId) {
        return this.query(`DELETE FROM roles WHERE role_id = $1`, [roleId])
    }
    deleteDepartment(departmentId) {
        return this.query(`DELETE FROM departments WHERE department_id = $1`, [departmentId])
    }
}
